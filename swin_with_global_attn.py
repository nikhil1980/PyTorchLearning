...
def group_by_key_prefix_and_remove_prefix(prefix, d):
    """
    This is to segregate each stage from s1 to s4

    :param prefix: string prefix to search
    :param d: entire string
    :return: tupe of dict of args without prefix
    :rtype: tuple
...
class PEG(nn.Module):
    def __init__(self, dim, kernel_size=3):
        super().__init__()
        self.proj = Residual(nn.Conv2d(dim,
                                       dim,
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(f"Inside Glob Attn q: {q.shape}, k: {k.shape}, v: {v.shape}")
        dots = einsum('b i d, b j d -> b i j', q, k) * self.scale
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads=8, dim_head=64, mlp_mult=4, local_patch_size=7, global_k=7, dropout=0.,
                 has_local=True, has_global=True):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual(LocalAttention(dim, heads=heads,
                                        dim_head=dim_head,
                                        dropout=dropout,
                ...
                Residual(GlobalAttention(dim,
                                         heads=heads,
                                         dim_head=dim_head,
                                         dropout=dropout,
                                         k=global_k)) if has_global else nn.Identity(),
                Residual(FeedForward(dim, mlp_mult, dropout=dropout)) if has_global else nn.Identity()
            ]))
...
'''
'We follow a sequential network
' Local attn / Identity 
'  |
'    ff 
'     |
'     global attn / Identity 
'      | 
'      ff
'''
...
class TwinsSVT(nn.Module):
    def __init__(
        ...
        for prefix in ('s1', 's2', 's3', 's4'):
            ...
            # Nikhil added selection flag to disable global attention on Stage 1
            is_first = prefix == 's1'
...
if __name__ == "__main__":
    ...
    model = TwinsSVT(
        ...
        # stage 1
        s1_emb_dim=64,  # Patch embedding projected dimension or C
        s1_patch_size=4,  # Patch size for patch embedding
        s1_local_patch_size=7,  # Patch size for local attention is fixed as 7
        s1_global_k=4,  # Global attention Value to key downsize ratio
        s1_depth=1,  # Number of transformer blocks

        # stage 2
        s2_emb_dim=128,  # 2*C
        s2_patch_size=2,  # 2X2 window
        s2_local_patch_size=7,
        s2_global_k=4,
        s2_depth=1,
